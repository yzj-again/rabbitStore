      ...
      <div class="bread-area">
        <rbread>
          ...
          <Transition name="fade-right" mode="out-in">
            <rbreaditem :key="breadData.sub.id" :to="`/category/sub/${breadData.sub.id}`">{{ breadData.sub.name }}</rbreaditem>
          </Transition>
        </rbread>
      </div>
      ...
      <div class="product-list-area" v-if="firstLoading&&filterListData.saleProperties.length>0">
        ...
        <div class="producct-filter-area">
          ...
          <div class="right-area">
            <rcheckbox v-model="cateProductFilter.inventory" @click="changeRightselect">仅显示有货商品</rcheckbox>
            <rcheckbox v-model="cateProductFilter.onlyDiscount" @click="changeRightselect">仅显示特惠商品</rcheckbox>
          </div>
        </div>
        <!--商品区域-->
...
export default {
  ...
  setup () {
    ...
    const breadData = computed(() => {
      const obj = {}
      store.state.category.cateList.forEach((item) => {
    ...
    const selectFilter = (filterName) => {
      if (filterName !== 'price') {
        ...
        cateProductFilter.sortMethod = null
      } else {
        // 次选是证明第一次点击
    ...
    const selectSpec = (parent, spec) => {
      if (parent.selectId === spec.id) return
      // 用户选中的规格id
    ...
    const userSelectBrand = (brand) => {
      if (filterListData.value.brands.selectId === brand.id) return
      const { id } = brand
      // 用户选中的品牌id
