      ...
      <div class="category-bread">
        <rbread>
          ...
          <Transition name="fade-right" mode="out-in">
            <rbreaditem :key="categoryData.id" :to="`/category/${categoryData.id}`">{{ categoryData.name }}</rbreaditem>
          </Transition>
        </rbread>
      </div>
      <!--轮播图组件-->
      <div class="category-swiper">
        <rswiper :swiperData="bannerList" v-if="bannerList.length>0"></rswiper>
...
export default {
  ...
  setup () {
    ...
    const getCateListData = async () => {
      ...
      document.title = '象米商城 - ' + result.name
