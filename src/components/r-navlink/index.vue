<template>
  <div class="r-nav-link-components clearfix">
    ...
    <div class="fl">
      <ul class="clearfix">
        <li v-for="(item,index) in navList" :key="index" @mousemove="showPopup(index)">
          <router-link :to="'/category/'+item.id" tag="a" :class="{'router-active':userActive===item.name}" @click="changeRoute(item)">{{ item.name }}</router-link>
        </li>
      </ul>
    </div>
    <!--搜索框-->
...
export default {
  ...
  setup (props) {
    ...
    // 根据组件类型进行样式的编写,控制盒子与navlink组件的距离
